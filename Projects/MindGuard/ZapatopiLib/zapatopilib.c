...
//					to_scan = gtk_timeout_add (3000, scan_for_signal, GINT_TO_POINTER(1));
					}
				else
					{
					append_log ("", gcar, 0, 0, "", false, gmode);
...
void create_biorhythm(void *widget)
{
	...
	if (pixmap_bio) gdk_pixmap_unref(pixmap_bio);
...
int diag_display_clear(void *data)
{
//	int i;
//	if (window)
//		for ( i=0 ; i < 5; i++)
...
void sort_mods()
{
	...
	if (mod_num > 0)
		...
		for (i = mod_num; i > 0; i--)
			...
			for (j = 0; j <= 255; j++)
				{
				if (module[i].name[j] == (int) NULL |
					module[i-1].name[j] == (int) NULL) return;
	...
	if ((file = fopen (path, "r")) != NULL)
		...
		while (fgets(word, 5, file) != NULL)
			...
			if (!strcmp(word, "NAME"))
				...
				module[num].name[len] = (int) NULL; /* add to make a proper string */
			...
			if (!strcmp(word, "(c) "))
				...
				module[num].copyright[len] = (int) NULL;
				word_offset(file, len);
				continue;
				}
			if (!strcmp(word, "VERS"))
				...
				module[num].version[len] = (int) NULL;
				word_offset(file, len);
				continue;
				}
			if (!strcmp(word, "LNUM"))
...
GtkWidget *make_web_link(char *url, int url_num)
{
	...
	gtk_signal_connect (GTK_OBJECT (button), "clicked",
			GTK_SIGNAL_FUNC (open_webpage), (gpointer) url_num);
...
void popup_dialog(const char *text, const char *ok, const char *cancel, void (*func_pointer_1) (void), void (*func_pointer_2) (void), int opts) {

    printf("%s - %s - %s", text, ok, cancel);
}
...
void popup_dialog(char *text, char *ok, char *cancel,
...
void open_logview_window()
{
	...
	if (window_logview == NULL)
		...
		gtk_box_pack_start(GTK_BOX(box_1), vscroll, false, false, 0);
...
{
	GtkWidget *box_about, *box_1, *box_2, *notebook, *label, *button, *separator, *table, *pix_mg;
	GdkPixmap *pixmap;
	GdkBitmap *mask;
	GtkStyle *style;
	char text[255];
	...
	if (window_about == NULL)
		...
		sprintf(text, "MindGuard X %s\nPersonal Mind Protection Software\n"
				...
				"Copyright \xa9 1999-%s Lyle Zapato\nAll Rights Reserved",
				mg_version, mg_year);
		...
		box_2 = make_web_link("http://zapatopi.net/mindguard.html", 2);
		...
		label = gtk_label_new ("Lyle Zapato v4.20\xdf\nCarbon Based Monkey Boy\n\n"
				"Copyright \xa9 1972 Mrs. Zapato\nAll Rights Reserved");
		gtk_box_pack_start (GTK_BOX(box_1), label, true, false, 0);
		gtk_widget_show (label);
...
void open_psident_window()
{
    ...
    // EPONYMOLOG
		...
		gtk_entry_set_text (GTK_ENTRY (entry_epo), user_name);
		gtk_signal_connect (GTK_OBJECT (entry_epo), "activate",
