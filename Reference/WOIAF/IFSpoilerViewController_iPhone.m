//
//  IFSpoilerViewController_iPhone.m
//  WOIAF
//
...
@interface IFSpoilerViewController_iPhone ()
...
@implementation IFSpoilerViewController_iPhone {
    ...
    // State control
    enum { hidden, shown, offscreen }   state;
...
- (void)viewDidLoad {
    ...
    //const CGFloat px = purchaseView.center.x;
    
    purchaseViewCentre[hidden] = CGPointMake(purchaseView.center.x, purchaseView.center.y);
    purchaseViewCentre[shown]  = CGPointMake(purchaseView.center.x, purchaseView.center.y - purchaseView.bounds.size.height);
...
- (void)offscreen:(id)sender {
    ...
    if (state == offscreen || stateTransition) return;
    ...
    [UIView animateWithDuration:.5
                     ...
                     animations:^{
                         ...
                         //self.view.superview.center = spoilerViewCentres[hidden];
                         
                         self.view.superview.center = CGPointMake(self.view.superview.center.x, self.view.superview.center.y + centreYDiffs[offscreen]);
                     }
...
- (void)hidden:(id)sender {
    ...
    [self.rootViewController lightenHomeScreen];
    
    // Animate the view onto the screen
    [UIView animateWithDuration:.5
                          delay:0
                        options:UIViewAnimationCurveEaseInOut /*(state == hidden) ? UIViewAnimationCurveEaseOut : UIViewAnimationCurveEaseIn*/
                     animations:^{
                         
                         stateTransition = YES;
                         //self.view.superview.center = spoilerViewCentres[shown];
                         if (state == shown) {
                             self.view.superview.center = CGPointMake(self.view.superview.center.x, self.view.superview.center.y + centreYDiffs[hidden]);
                         }else{
                             self.view.superview.center = CGPointMake(self.view.superview.center.x, self.view.superview.center.y - centreYDiffs[offscreen]);
...
- (void)shown:(id)sender {
    ...
    [UIView animateWithDuration:.5
                     ...
                     animations:^{
                         ...
                         //self.view.superview.center = spoilerViewCentres[grown];
...
- (IBAction)viewGeorgesPageInBookstore:(UIButton *)sender {
    ...
    //https://itunes.apple.com/us/artist/george-r.-r.-martin/id2067983?mt=11#
