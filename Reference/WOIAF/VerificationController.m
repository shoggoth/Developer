...
- (NSDictionary *)dictionaryFromPlistData:(NSData *)data
{
    ...
    if (!dictionaryParsed)
    {
        if (error) NSLog(@"Dictionary parse error %@", error);
...
#ifdef USE_IAP_SANDBOX
#warning Using sandbox for in-app purchases
...
- (void)connection:(NSURLConnection *)connection didReceiveData:(NSData *)data
{
    ...
    [self.delegate transaction:responseString verifiedWithAppStore:isOk];
    
    if (isOk)
    {
        //Validation suceeded. Unlock content here.
//        NSLog(@"Validation success for %@", responseString);
...
{
    // Get the base string
    NSString* baseString = [NSString stringWithUTF8String:base64_encode(input, (size_t)length)];
...
- (NSString *)decodeBase64:(NSString *)input length:(NSInteger *)length
{
    ...
    //NSString *st = [NSString stringWithUTF8String:retStr];
...
void* base64_decode(const char* s, size_t* data_len_ptr)
{
    ...
    if (len % 4) [NSException raise:@"Invalid input in base64_decode" format:@"%zd is an invalid length for an input string for BASE64 decoding", len];
